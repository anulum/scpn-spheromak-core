...
# SCPN Spheromak Core — device geometry model tests
...
def test_derived_outer_radius() -> None:
    """The gun outer electrode's outer radius is the bore plus the wall."""
    geometry = reference_geometry()
    assert geometry.gun_outer_electrode_outer_radius_m == 0.38 + 0.02
...
def test_inner_electrode_must_fit_the_gun_bore() -> None:
    """An inner electrode as wide as the outer electrode bore is refused."""
    with pytest.raises(DeviceGeometryError, match="gun_inner_electrode_radius_m"):
        dataclasses.replace(reference_geometry(), gun_inner_electrode_radius_m=0.38)
