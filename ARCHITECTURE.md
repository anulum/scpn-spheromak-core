...
SCPN Spheromak Core — Architecture summary
-->
...
(`src/scpn_spheromak_core/`, ADR 0002) — alongside the device boundary, its
...
Boundary in one paragraph: this repository owns spheromak plant and
experiment truth — configuration policy for simply connected compact
toroids near a Taylor relaxed state, coaxial-gun formation and
helicity-injection sustainment semantics, pulsed lifecycle definitions with
tilt/shift and helicity-balance records, magnetics diagnostic and clock
declarations in the flux-conserver frame, actuator-response boundaries,
safety-envelope declarations, and the device-owned CONTROL adapter
specification. Solver mathematics stays in `SCPN-FUSION-CORE`; typed
