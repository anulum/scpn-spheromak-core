...
# SCPN Spheromak Core — diagnostic and clock semantics
...
identities for the spheromak configuration this repository owns,
...
from scpn_spheromak_core.configuration import ConsistencyFinding
...
TILT_SHIFT_BAND_HZ: Final = (1.0e03, 2.0e05)

SHOT_DURATION_CEILING_S: Final = 1.0e-01
        ...
        for configuration in self.configurations:
            if configuration not in OWNED_CONFIGURATIONS:
                raise DiagnosticPlanError(
                    ...
                    f"{configuration!r} is not owned by SCPN-SPHEROMAK-CORE"
                )
...
class DiagnosticChannelPlan:
    ...
    timing_uncertainty_s
        ...
        candidate is applicable to SCPN-SPHEROMAK-CORE.
    acquisition_start_s
    ...
    def __post_init__(self) -> None:
        ...
        if self.candidate_id not in _CANDIDATE_INDEX:
            raise DiagnosticPlanError(
                ...
                "applicable to SCPN-SPHEROMAK-CORE"
            )
        if not self.reason:
            ...
            if channel.candidate_id == "closed.resolved_mhd_mode" and not (
                ...
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            f"declared tilt/shift band "
                            f"{channel.max_signal_frequency_hz:.1f} "
                            "Hz is outside the spheromak tilt/shift and "
                            "relaxation activity scale 1-200 kHz (Bellan 2000)"
                        ),
                    )
                )
                ...
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            ...
                            "longer than the spheromak decay scale of up to "
                            "~0.1 s (Bellan 2000)"
                        ),
                    )
                )
