...
# SCPN Spheromak Core — device CAD model record (tier G2)
...
electrodes, end wall) into the same five named bodies as the tier-G1
model (:func:`build_device_model`), built as exact B-rep solids of
revolution by the pinned third-party OpenCASCADE kernel through the
shared kernel library (``scpn_reactor_kernels.cad``, kernels
...
of the five bodies, the build invariants of this family and its
...
def build_device_cad(
    ...
    ------
    DeviceGeometryError
        If the segment count is invalid, the layout invariants fail, a
    ...
    try:
        assembly = BrepAssembly(
            (
                cylinder_solid_brep(
                    geometry.gun_inner_electrode_radius_m,
                    z_gun_low,
                    0.0,
                    BODY_GUN_INNER_ELECTRODE,
                    ROLE_ELECTRODE,
