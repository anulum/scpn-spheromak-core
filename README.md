...
the device 3D model — the validated device geometry of the gun-driven
assembly tessellated deterministically into closed triangle meshes of five
analytic bodies, exported as binary STL and glTF 2.0 binary and proven
bit-exact against the library's native module; and the device CAD model —
the same five bodies as B-rep solids built by the pinned OpenCASCADE
kernel through the library's `cad` group, checked fail-closed by the
library's evidence kernel against the analytic closed forms, exported as a
normalised deterministic STEP file whose digest the model record carries
with the back-end versions (ADR 0007, evidence:
...
- the device geometry and its 3D models: the gun-driven mechanical
  envelope (flux-conserver wall, coaxial gun electrodes, end wall) and
  the deterministic analytic-surface and B-rep models derived from it,
  with open-format exports (no engineering property carried); the
  tessellation, mesh, export, CAD and evidence kernels themselves are the
